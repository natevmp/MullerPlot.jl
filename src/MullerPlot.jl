...
Create a vector `vid_child_Vid`, indexed by the variant id's `vid`, that has as elements the children of each variant stored as a vector of id's. For example, `vid_child_Vid[5]` is a vector `vid_child` representing the children of variant with `vid=5`, where the elements are the `vid`'s of the respective children.
